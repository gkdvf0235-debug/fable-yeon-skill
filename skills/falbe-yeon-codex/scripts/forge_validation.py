...
EXTERNAL_EFFECT_FIELDS = ("action", "target", "impact", "rollback", "approval")
...
SPEC_TEMPLATE = {
    ...
    "ssot_decisions": [],           # {concept, authority, decision}
    "ssot_ambiguities": [],         # {concept, candidates, recommended_authority, resolution, authority}
    "external_effects": [],         # {action, target, impact, rollback, approval}
...
def detect_external_effect(command: str) -> str:
    """Classify shell commands that can affect the outside world or destroy state.

    This intentionally catches only clear, high-signal commands. It is used by the
    PreToolUse hook while Fable is active, so false positives cost a turn; keep it
    conservative and expand only with tests.
    """
    raw = command or ""
    text = _norm(raw)
    if not text:
        return ""
    if "--dry-run" in text or "--what-if" in text or "-whatif" in text or " -whatif" in text:
        return ""
    if re.search(r"\bgit\s+push\b", text):
        return "git_push"
    if re.search(r"\b(?:vercel|netlify|firebase|wrangler)\b.*\b(?:deploy|publish)\b", text):
        return "deploy"
    if re.search(r"\b(?:prisma|alembic|sequelize|knex|typeorm|rails|django-admin)\b.*\bmigrat", text):
        return "migration"
    if re.search(r"\brm\s+-[^\n]*r[^\n]*f\b|\bremove-item\b[^\n]*(?:-recurse|-r)\b|\brmdir\b[^\n]*(?:/s|-r)\b", text):
        return "destructive_delete"
    if re.search(r"\b(?:aws|gcloud|az)\b.*\b(?:deploy|publish|sync|cp|rm|delete|put-object)\b", text):
        return "cloud_write"
    if re.search(r"\b(?:stripe|paypal)\b.*\b(?:create|update|delete|charge|refund|payment|invoice)\b", text):
        return "payment"
    if re.search(r"\b(?:sendgrid|mailgun|resend|smtp)\b.*\b(?:send|deliver|mail|email)\b", text):
        return "email"
    return ""

def _external_effect_shape_errors(item, idx: int) -> list[str]:
    if not isinstance(item, dict):
        return [f"external_effects[{idx}] must be an object {{{','.join(EXTERNAL_EFFECT_FIELDS)}}}."]
    missing = [field for field in EXTERNAL_EFFECT_FIELDS if not _nonempty(item.get(field))]
    e = [f"external_effects[{idx}] needs {', '.join(missing)}."] if missing else []
    action = str(item.get("action") or "").strip().lower()
    if action and action not in EXTERNAL_EFFECT_ACTIONS and action not in {"*", "any"}:
        e.append(
            f"external_effects[{idx}].action '{action}' not in {sorted(EXTERNAL_EFFECT_ACTIONS)} or '*'."
        )
    return e

def _authorized_external_effect(spec: dict, action: str) -> bool:
    for item in spec.get("external_effects", []):
        if _external_effect_shape_errors(item, -1):
            continue
        declared = str(item.get("action") or "").strip().lower()
        if declared in {action, "*", "any"}:
            return True
    return False

def gate_external_effect(spec: dict, command: str) -> list[str]:
    action = detect_external_effect(command)
    if not action:
        return []
    if _authorized_external_effect(spec, action):
        return []
    return [
        f"external effect '{action}' blocked — add external_effects entry "
        "{action,target,impact,rollback,approval} before running this command."
    ]
...
def gate_spec(spec: dict, root=None) -> list[str]:
    ...
    for i, d in enumerate(spec.get("ssot_decisions", [])):
        if not isinstance(d, dict):
            e.append(f"ssot_decisions[{i}] must be an object {{concept,authority,decision}}.")
        elif not (_nonempty(d.get("concept")) and _nonempty(d.get("authority")) and _nonempty(d.get("decision"))):
            e.append(f"ssot_decisions[{i}] needs concept + authority + decision.")

    for i, a in enumerate(spec.get("ssot_ambiguities", [])):
        if not isinstance(a, dict):
            e.append(f"ssot_ambiguities[{i}] must be an object.")
            continue
        candidates = a.get("candidates")
        has_candidate = isinstance(candidates, list) and any(_nonempty(str(x)) for x in candidates)
        if not (_nonempty(a.get("concept")) and has_candidate and _nonempty(a.get("recommended_authority"))):
            e.append(
                f"ssot_ambiguities[{i}] needs concept + candidates + recommended_authority."
            )
        if not (_nonempty(a.get("resolution")) and _nonempty(a.get("authority"))):
            e.append(
                f"ssot_ambiguities[{i}] unresolved — add resolution + authority before implementation."
            )

    for i, item in enumerate(spec.get("external_effects", [])):
        e.extend(_external_effect_shape_errors(item, i))

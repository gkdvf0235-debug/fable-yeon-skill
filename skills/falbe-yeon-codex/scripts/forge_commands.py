...
def _contract_text(grade: str) -> str:
    ...
    std = [
        ...
        "- ssot_decisions: optional; when a task touches a contract/policy/schema/config/type/prompt/design-token SSOT, record {concept, authority, decision}.",
        "- ssot_ambiguities: optional, but any entry means an SSOT conflict was found and must be resolved; each needs {concept, candidates, recommended_authority, resolution, authority}. An unresolved entry is blocked.",
        "- external_effects: optional until you run an outside-world command; before git push/deploy/migration/destructive delete/email/payment/cloud write, add {action,target,impact,rollback,approval}.",
...
def cmd_external_effect(args) -> int:
    root = Path(args.root).resolve()
    command = args.command or ""
    action = detect_external_effect(command)
    if not action:
        print("forge external-effect gate: PASS (no external effect detected)")
        return 0
    spec, err = load_spec(root)
    if err:
        record_loop_event(root, "validate_block", gate="external_effect", message=err)
        print(f"forge external-effect gate: BLOCKED\n  - {err}", file=sys.stderr)
        return 1
    errs = gate_external_effect(spec, command)
    if errs:
        record_loop_event(root, "validate_block", gate="external_effect", message="\n".join(f"- {x}" for x in errs))
        print(f"forge external-effect gate: BLOCKED ({action})", file=sys.stderr)
        for x in errs:
            print(f"  - {x}", file=sys.stderr)
        print(f"  supported actions: {sorted(EXTERNAL_EFFECT_ACTIONS)}", file=sys.stderr)
        return 1
    record_loop_event(root, "validate_pass", gate="external_effect", status=action)
    print(f"forge external-effect gate: PASS ({action})")
    return 0
...
def main(argv=None) -> int:
    ...
    ee = sub.add_parser("external-effect"); ee.add_argument("--root", required=True)
    ee.add_argument("--command", required=True)
    ee.set_defaults(fn=cmd_external_effect)
    ...
    try:
        ...
        fail_closed = getattr(args, "cmd", "") in ("validate", "close", "external-effect")
